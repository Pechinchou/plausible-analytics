defmodule PlausibleWeb.Live.RegisterForm do
    ...
    if socket.assigns.live_action == :register_from_invitation_form and
      ...
      {:ok,
       assign(socket,
         ...
         heading: "Convite não é mais válido",
         subtitle:
           "Este convite expirou ou foi revogado. Peça ao administrador da sua equipe para enviar um novo convite."
       )}
    else
      changeset =
  ...
  defp heading_and_subtitle(:register_from_invitation_form) do
    {"Crie sua conta", "Aceite o convite para entrar na sua equipe."}
  end

  defp heading_and_subtitle(:register_form) do
    if ce?() do
      {"Crie sua conta #{Plausible.product_name()}",
       "Comece a rastrear análises com privacidade em minutos."}
    else
      {"Inicie seu período de teste gratuito de 30 dias",
